...
def validate_location(location_name: str) -> bool:
    ...
    completion = client.chat.completions.parse(
        model=model,
        messages=[
            {"role": "system", "content": "Determine if this is a geographical location."},
    ...
    return response.is_valid_location
...
def get_function(location_name: str) -> tuple:
    ...
    func_args = json.loads(completion.choices[0].message.tool_calls[0].function.arguments)
...
def main():
    ...
    # 1. Get and validate user input for location (openai) (pydantic) (OpenAI API : 1st Call)
    location_name = input_location()

    # 2. Get call funcation name and arguments (location coordinates) (OpenAI API : 2nd Call)
